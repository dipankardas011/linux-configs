...
# Function to get the current git branch (if applicable)
git_branch() {
    branch=$(git branch 2>/dev/null | grep '*' | sed 's/* //')
    [ -n "$branch" ] && echo " (${YELLOW}${branch}${WHITE})"
}
...
# Emoji indicator for root or user
if [ $EUID = 0 ]; then
    prompt_icon="🔥"  # Root indicator
    user_color=$RED
else
    prompt_icon="💻"  # User indicator
    user_color=$BLUE
fi
